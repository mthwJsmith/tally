...
impl Db {
    ...
    // ============================================================
    // v2 — OAuth 2.1 (MCP authorization server)
    // ============================================================
    ...
    /// Returns the client's registered redirect URIs (raw JSON array string), if it exists.
    pub async fn get_oauth_client_redirects(&self, client_id: &str) -> Result<Option<String>> {
        Ok(
    ...
    pub async fn create_auth_code(
        ...
        let now = Utc::now().timestamp();
        sqlx::query(
            "INSERT INTO oauth_auth_codes (code, client_id, redirect_uri, code_challenge,
                code_challenge_method, user_id, scope, resource, expires_at, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(code)
        .bind(client_id)
    ...
    /// Atomically consume an auth code (one-time use). Returns
    /// (client_id, redirect_uri, code_challenge, user_id, scope) if valid + unexpired.
    pub async fn take_auth_code(
        &self,
        code: &str,
    ) -> Result<Option<(String, String, String, i64, Option<String>)>> {
        ...
        .fetch_optional(&self.pool)
        .await?;
        // Always delete (one-time), even if expired.
        sqlx::query("DELETE FROM oauth_auth_codes WHERE code = ?")
            .bind(code)
            .execute(&self.pool)
            .await?;
        match row {
            Some((cid, ru, cc, uid, scope, exp)) if exp >= Utc::now().timestamp() => {
                Ok(Some((cid, ru, cc, uid, scope)))
            }
            _ => Ok(None),
        }
    }
    ...
    pub async fn create_oauth_token(
        ...
        let now = Utc::now().timestamp();
        sqlx::query(
            "INSERT INTO oauth_access_tokens (token_hash, refresh_token_hash, client_id, user_id,
    ...
    /// Validate an OAuth access token hash → user_id, if unexpired and not revoked.
    pub async fn validate_oauth_token(&self, access_hash: &str) -> Result<Option<i64>> {
    ...
    /// Consume a refresh token (rotate): returns (client_id, user_id, scope) and revokes the
    /// old row. The caller issues a fresh access+refresh pair.
    pub async fn take_refresh_token(
        ...
        let row: Option<(String, i64, Option<String>)> = sqlx::query_as(
            "SELECT client_id, user_id, scope FROM oauth_access_tokens
        ...
        if row.is_some() {
            sqlx::query(
                "UPDATE oauth_access_tokens SET revoked_at = ? WHERE refresh_token_hash = ?",
            )
            .bind(Utc::now().timestamp())
            .bind(refresh_hash)
            .execute(&self.pool)
