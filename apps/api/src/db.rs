...
impl Db {
    ...
    pub async fn list_watchlist(&self) -> Result<Vec<WatchlistItem>> {
        Ok(sqlx::query_as::<_, WatchlistItem>(
            "SELECT * FROM watchlist_items WHERE archived = 0 ORDER BY created_at DESC",
        )
        .fetch_all(&self.pool)
        .await?)
    }
    ...
    pub async fn create_watchlist_item(
        ...
        let now = Utc::now().timestamp();
        let row: (i64,) = sqlx::query_as(
            "INSERT INTO watchlist_items (name, keywords, target_price_cents, currency, created_at)
             VALUES (?, ?, ?, ?, ?) RETURNING id",
        )
        .bind(name)
        .bind(keywords)
        .bind(target_price_cents)
        .bind(currency)
        .bind(now)
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn add_watchlist_source(&self, item_id: i64, kind: &str, r#ref: &str) -> Result<i64> {
        let now = Utc::now().timestamp();
        let row: (i64,) = sqlx::query_as(
            "INSERT INTO watchlist_sources (item_id, kind, ref, created_at)
             VALUES (?, ?, ?, ?) RETURNING id",
        )
        .bind(item_id)
        .bind(kind)
    ...
    pub async fn watchlist_sources(&self, item_id: i64) -> Result<Vec<WatchlistSource>> {
        Ok(sqlx::query_as::<_, WatchlistSource>(
            "SELECT * FROM watchlist_sources WHERE item_id = ?",
        )
        .bind(item_id)
        .fetch_all(&self.pool)
        .await?)
    ...
    pub async fn delete_watchlist_item(&self, id: i64) -> Result<()> {
        // Sources + observations cascade via the FK.
    ...
    pub async fn recent_deals(&self, limit: i64) -> Result<Vec<DealObservation>> {
        Ok(sqlx::query_as::<_, DealObservation>(
            "SELECT * FROM deal_observations ORDER BY found_at DESC LIMIT ?",
        )
    ...
    /// Insert an observation, returning its id, or `None` if it already exists (dedup on guid).
    #[allow(clippy::too_many_arguments)]
    pub async fn insert_observation_if_new(
        &self,
        item_id: i64,
        title: &str,
        url: Option<&str>,
        ...
        let row: Option<(i64,)> = sqlx::query_as(
            "INSERT OR IGNORE INTO deal_observations
                (item_id, title, url, price_cents, source_kind, guid, found_at, notified)
    ...
    pub async fn mark_deal_notified(&self, id: i64) -> Result<()> {
        sqlx::query("UPDATE deal_observations SET notified = 1 WHERE id = ?")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(())
